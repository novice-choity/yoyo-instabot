"""
Permission Mixins that controls the Specific type of user entry for any particular view
"""
...
class AccessMixin(object):
    ...
    def handle_no_permission(self, request):
        if self.raise_exception:
            if self.redirect_unauthenticated_users and not _is_authenticated(request.user):
...
#
#     def dispatch(self, request, *args, **kwargs):
#         if not request.user.is_staff or not (request.user.user_type == OWNER):
#             return self.handle_no_permission(request)
#
#         return super(OwnerRequiredMixin, self).dispatch(request, *args, **kwargs)


class SystemAdminRequiredMixin(SuperuserRequiredMixin):
    """
    Mixin class that controls the system admin or superuser access
    """
    pass
